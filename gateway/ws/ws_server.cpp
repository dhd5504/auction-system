...
void sendRaw(const std::shared_ptr<Pistache::Tcp::Peer>& peer, const std::string& data) {
    if (!peer) return;
    peer->send(Pistache::RawBuffer(data.data(), data.size()));
}
...
void WsServer::handleHandshake(const std::shared_ptr<Pistache::Tcp::Peer>& peer, ClientState& state) {
    ...
    std::istringstream stream(buffer.substr(0, pos + 4));
    std::string line;
    std::string websocketKey;
    while (std::getline(stream, line)) {
        line = trim(line);
        if (line.rfind("Sec-WebSocket-Key:", 0) == 0) {
            websocketKey = trim(line.substr(strlen("Sec-WebSocket-Key:")));
        }
    }

    if (websocketKey.empty()) {
        sendRaw(peer, "HTTP/1.1 400 Bad Request\r\n\r\n");
        state.buffer.clear();
        return;
    }

    std::string magic = websocketKey + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
    auto digest = sha1Bytes(magic);

    std::string acceptKey = base64Encode(std::string(reinterpret_cast<char*>(digest.data()), digest.size()));
    ...
    sendRaw(peer, resp.str());
    state.handshaken = true;
    state.buffer.erase(0, pos + 4);
}
...
void WsServer::handleFrames(const std::shared_ptr<Pistache::Tcp::Peer>& peer, ClientState& state) {
    ...
    while (buffer.size() >= 2) {
        ...
        if (payloadLen == 126) {
            ...
            payloadLen = (static_cast<uint8_t>(buffer[offset]) << 8) | static_cast<uint8_t>(buffer[offset + 1]);
            offset += 2;
        ...
        if (masked) {
            ...
            uint8_t mask[4] = {
                ...
                static_cast<uint8_t>(buffer[offset + 2]),
                static_cast<uint8_t>(buffer[offset + 3])};
            offset += 4;

            if (buffer.size() < offset + payloadLen) return;
